...
SDL_Texture* gImageCurrent = NULL;
...
//***free*************************
void freeF(SDL_Renderer* Main_Renderer, SDL_Texture* gImageCurrent);
...
printf("in simplyUploadAMap: 12\n");
		...
		//Apply the image
		SDL_RenderCopy( Main_Renderer, gImageCurrent, NULL, NULL );
    ...
    //SDL_RenderCopy(Main_Renderer, gImageCurrent, NULL, NULL);
		...
		SDL_Delay( 1000 );
		//**free*************************
printf("in simplyUploadAMap: 17\n");
		//freeF(Main_Renderer, gImageCurrent);
printf("in simplyUploadAMap: 18\n");
	}
...
bool init() {
	...
	if( SDL_Init( SDL_INIT_VIDEO ) < 0 ) {
		...
		//Create window
		Main_Window = SDL_CreateWindow( "World Map", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( Main_Window == NULL ) {
			if ( DEBUG_LEVEL >= debug_test ) {
			...
			/***BMP ERA IMAGE*******************************/
			//Get window surface
			//Main_Renderer = SDL_GetWindowSurface( Main_Renderer );
...
///	gImageCurrent = SDL_LoadBMP( gImageCurrent_name );
	Loading_Surf = SDL_LoadBMP( gImageCurrent_name );
printf("23, in loadMedia: gImageCurrent_name: %s\n", gImageCurrent_name);
	gImageCurrent = SDL_CreateTextureFromSurface(Main_Renderer, Loading_Surf);
...
printf("in heroWalking: 47\n");
		SDL_RenderCopy( Main_Renderer, gImageCurrent, NULL, &gImageCurrentPosition );
		//Update the surface
		SDL_RenderPresent( Main_Renderer );
