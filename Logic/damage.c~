...
void implementDamage(int attack, enemy *enemy, int* playerHealth);
...
int damageState(int attack, enemy *enemy, int *playerHealth, battleState *pState)
{
    implementDamage(attack, enemy, playerHealth);
    return checkHealth(enemy->health, *playerHealth, pState);
}

/*Checks if an attack was successful and damages player or enemy accordingly*/
void implementDamage(int attack, enemy *enemy, int* playerHealth)
{
    if(checkAttack(attack, enemy)){
        enemy->health--;
    }
    else{
        (*playerHealth)--;
